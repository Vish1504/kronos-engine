...
{ 
    ...
    std::string value = configurations_.at("memtable_size_mb");        
        ...
        throw std::runtime_error("Invalid input: No conversion could be performed.\n");
    } 
    catch (const std::out_of_range& e) {
        throw std::runtime_error("Overflow error: Value out of range for an int.\n");
    }
}
...
std::string kronos::Config::getLogPath(){
    ...
    return value;
    throw std::runtime_error(
        "Configuration error: 'logPath' key is missing.\n"
    );
