...
// making this helper private to sstable.cpp
namespace {
void fsyncParentDirectory(const std::filesystem::path &path) {
  ...
  if (fd == -1) {
    throw std::runtime_error(
        "Failed to open SSTable parent directory for fsync");
  }
  ...
  while (::fsync(fd) == -1) {
    ...
    throw std::runtime_error("Failed to fsync SSTable parent directory");
  }
...
void kronos::SstableBuilder::finish() {
  ...
  /*
   * Persist the completed temporary SSTable before making its final
   * filename visible.
   */
  while (::fsync(fd_) == -1) {
    if (errno == EINTR) {
  ...
  if (::close(fd_) == -1) {
    ...
    throw std::runtime_error("Failed to close SSTable");
  }
   ...
   *
   * before rename -> incomplete/new SSTable is not live
   * after rename  -> complete .sst file exists
   *
   * MANIFEST still decides whether that SSTable is logically authoritative.
   */
  if (std::rename(temp_path.c_str(), sstable_path_.c_str()) != 0) {
    throw std::runtime_error("Unable to rename SSTable .tmp to .sst");
  }

  /*
   * Persist the directory-entry change caused by rename.
   *
   * Some platforms/filesystems do not support directory fsync; those cases
   * are treated as unsupported rather than corruption.
   */
  fsyncParentDirectory(sstable_path_);
}
