...
//   //   if writeHeader() fails, construction itself fails, and C++ will not
//   call
...
//               *>(currentBlock_recordCount_bytes),
...
//   bloom_block.insert(bloom_block.end(), bit_count_bytes, bit_count_bytes +
//   4);
...
//                        firstKey_length_bytes +
//                        sizeof(firstKey_length_bytes));
...
// // final block -> bloom filter -> sparse index -> footer -> fsync -> rename
// .tmp
// // to .sst
// // Finalize the SSTable in strict order:
// // final block -> bloom filter -> sparse index -> footer -> fsync -> rename
...
// // Reconstruct an unsigned integer from its little-endian disk
// representation. template <typename T> T decode_from_le(const uint8_t arr[]) {
//   static_assert(std::is_unsigned_v<T>);
...
//   uint32_t entry_count = decode_from_le<uint32_t>(index_bytes.data() +
//   cursor);

//   cursor += sizeof(uint32_t);

//   // Minimum entry = key_len(4) + empty key(0) + offset(8) + size(4) = 16
//   bytes.
...
//     uint32_t block_size = decode_from_le<uint32_t>(index_bytes.data() +
//     cursor);
...
//   // Sparse index gives us the exact disk region containing the candidate
//   block. uint64_t target_block_offset =
//   sparse_index_[target_id].block_offset; uint32_t target_block_size =
//   sparse_index_[target_id].block_size;
...
//   uint32_t record_count = decode_from_le<uint32_t>(block_bytes.data() +
//   cursor);
...
//     uint64_t sequence = decode_from_le<uint64_t>(block_bytes.data() +
//     cursor);
...
//     uint32_t key_length = decode_from_le<uint32_t>(block_bytes.data() +
//     cursor);
...
//         return {.value = std::move(record_value), .status =
//         GetStatus::FOUND};
//       }
...
//   }
//   if (cursor != payload_size) {
//     throw std::runtime_error("Unexpected trailing bytes in SSTable data
//     block");
//   }
...
//   uint64_t sequence = decode_from_le<uint64_t>(block_bytes_.data() +
//   cursor_);
...
//   uint32_t key_length = decode_from_le<uint32_t>(block_bytes_.data() +
//   cursor_);
...
//   current_key_ =
//       std::string(reinterpret_cast<const char *>(block_bytes_.data() +
//       cursor_),
//                   key_length);
 ...
 * 4. Sequentially scan its sorted records for the exact key.
 */
std::optional<kronos::InternalEntry>
kronos::SstableReader::lookupEntry(const std::string &key) const {
  ...
  // Sparse index identifies the candidate data block containing this key.
  ...
  uint32_t calculated_checksum = static_cast<uint32_t>(crc);
  const uint8_t *stored_crc_ptr = block_bytes.data() + payload_size;
  uint32_t stored_checksum = decode_from_le<uint32_t>(stored_crc_ptr);
  ...
  // Minimum serialized record size:
  // sequence(8) + operation(1) + key_len(4) + value_len(4) = 17 bytes.
  size_t remaining_bytes = payload_size - cursor;
  if (record_count > remaining_bytes / 17) {
    throw std::runtime_error("Invalid SSTable record count");
  }
  ...
  for (uint32_t i = 0; i < record_count; i++) {
    ...
    if (record_key == key) {
      ...
      } else if (operation_byte == static_cast<uint8_t>(OperationType::PUT)) {
        operation = OperationType::PUT;
      } else {
        throw std::runtime_error("Invalid operation type in SSTable record");
      }

      return InternalEntry{std::move(record_value), sequence, operation};
    }

    // Records inside the block are sorted. Once we pass the target key it
    // cannot appear later in this candidate block.
    if (record_key > key) {
      return std::nullopt;
    }
  }
...
kronos::SstableReader::get(const std::string &key) const {
  ...
  return {.value = std::move(entry->value), .status = GetStatus::FOUND};
}
